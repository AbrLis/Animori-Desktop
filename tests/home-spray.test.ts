// Проверки россыпи на плашке приветствия (`app/screens/home-spray`):
// число цветков обязано расти вместе со свободным местом, цветок не
// обязан залезать на строку или на розетку, а повторный расчёт при
// неизменном размере не должен переставлять россыпь.
//
// Всё это ломается тихо и видно только глазами: россыпь, посчитанная
// по ширине окна, а не по свободному месту, оставляла пустоту на широком
// экране и налезала на строку на узком.

// Проверки витрины рекомендаций (`core/recs`): полка обязана оставаться
// заполненной после отметок «не интересно», а очистка памяти обязана
// эти отметки снимать.
//
// Оба случая ломаются тихо и видны только на живом экране: полка, из
// которой выщелкали всё, просто пропадала, а снятие метки не возвращало
// аниме до перезапуска. Здесь они заперты.
//
// Каталог подменён целиком: проверяется раскладка полок, а не сеть.
// Модуль берётся заново на каждый случай — запасы полок живут в памяти
// модуля, и иначе соседние проверки делили бы один запас.

...
function frame(grid: Uint8Array, fixed: Uint8Array, size: number, version: number): void {
  ...
  // Шестая клетка пропускается нарочно: на ней сведения о формате не лежат,
  // там полоса синхронизации. Записать в неё ноль — значит разорвать полосу
  // пополам ровно посредине: читатель ждёт на шестой строке и шестом столбце
  // чередование и по перекосу решает, что сетка уехала.
  for (let i = 0; i < 9; i++) {
    if (i === 6) continue

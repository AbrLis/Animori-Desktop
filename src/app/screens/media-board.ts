...
//
// Высоту плитке выдаёт только align-self: stretch. Процентная высота
// здесь ядовита: при измерении плитка стоит в строке в один шаг сетки,
// height: 100% считался от него, и все панели мерились в 8 пикселей —
// содержимое вываливалось наружу и налезало на соседние плитки.
//
...
/** Метка плитки, которую тянули ниже её собственной высоты. По ней CSS
 *  раздаёт добор содержимому — например, снимает потолок у списка
 *  хронологии. На время измерения метка снимается: иначе франшиза
 *  без потолка показала бы натуральную высоту во всю длину списка. */
const GROWN = 'am-board__grown'
...
function lay(board: HTMLElement): void {
  ...
  // и метку добора прошлой перекладки, иначе вместо натуральной высоты
  // мы измерим её же вчерашний добор и плитки будут расти без конца.
  ...
  // до него и получает метку добора: по ней CSS отдаёт лишнюю высоту
  // содержимому плитки, а не воздуху под ним.

// Россыпь мелких сакур на плашке приветствия.
//
// ЧИСЛО ИЗ СВОБОДНОГО МЕСТА
// Плашка — широкая низкая полоса, и свободно на ней ровно столько,
// сколько не заняли строка с кнопками и крупная розетка справа. Поэтому
// цветы не расставлены раз и навсегда: плашка делится на клетки, клетка,
// задевающая текст или розетку, отбрасывается, в остальные садится по
// цветку. Шире окно — больше клеток — гуще россыпь; длинная фраза
// отъедает клетки у строки, и россыпь редеет сама.
//
// ПОЧЕМУ ШУМ СЧИТАЕТСЯ, А НЕ БЕРЁТСЯ ИЗ Math.random
// Наблюдатель за размером вызывает расчёт при всяком изменении плашки.
// Случайные числа дали бы новую раскладку на каждый кадр перетаскивания
// окна — россыпь мигала бы. Значение выводится из номера клетки и соли
// запуска: расчёт устойчив к повтору и всё же даёт новый рисунок на
// каждый запуск.
//
// ТРИ ГЛУБИНЫ
// Крупные бледные сзади, мелкие плотные спереди — плоская россыпь одного
// размера читается наклейками. Глубина же задаёт и движение: дальние
// плывут медленнее и на меньшее расстояние, ближние быстрее, отчего
// полоса получает подобие объёма.
...
// Размах и длительность подобраны по видимости, а не по вкусу: прежние
// «пять пикселей за полминуты» давали 0,17 px/с, и движения просто не
// было видно. Сейчас ближний цветок проходит свои два десятка пикселей
// секунд за десять — глаз это берёт, а полоса остаётся спокойной.
// Дальний плывёт вдвое медленнее и вдвое меньшим размахом: это и есть
// подобие объёма, ради которого россыпь вообще разделена на глубины.
...
/**
 * Раскладка россыпи для плашки размера `w` × `h`.
 *
 * `shut` — занятые области: строка с кнопками и крупная розетка.
 */
